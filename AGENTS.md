...
- **Scope `auth.interceptor.ts`/`error.interceptor.ts` logic to `req.url.startsWith(environment.apiBaseUrl)`.** Both must pass any other request through completely untouched. This is not optional: the direct-to-Cloudinary upload sends requests to a third-party origin, and without the guard, `auth.interceptor.ts` would leak our JWT to it, and `error.interceptor.ts` would run Cloudinary's error body through `mapApiError` — which assumes our own DRF shapes — silently discarding the real error into a lossy `{type:'unknown'}`.
- **Don't add `provideAnimationsAsync()`/`provideNoopAnimations()` when introducing an Angular Material component, unless a real build or runtime error demonstrates one is actually needed.** `@angular/animations` is not installed in this project and is not a peer dependency of `@angular/material` at the installed version — `MatSnackBar` (and, unless proven otherwise, other Material components) works with zero animations provider configured. Adding either provider without verifying first breaks the build (`Could not resolve "@angular/animations/browser"`), since both live in the same module with a static top-level import of that path regardless of which function is called.
...
- **Each commit is a vertical slice**, not a layer: component/service code, its `i18n` strings and Spanish translations, its unit tests, its E2E spec, **and its documentation** land together. Avoid horizontal commits ("all translations", "all tests", "all docs").
- **Documentation is part of the slice, not a follow-up.** Every commit that adds a real finding, a scope decision, a verified behavior, or a corrected assumption gets a matching update in the same commit: `PLAN.md` (an "— implemented and verified" narrative section per commit, matching the existing per-commit sections — edited on disk, but gitignored/uncommitted per its own scope) and `docs/ARCHITECTURE.md` (a new or extended section when the finding is architectural, not just a one-off implementation detail). A pure mechanical change (a rename, a formatting pass, a dependency bump with nothing to explain) doesn't need one. This was skipped for commits 14-15 and the E2E-suite/test-cleanup work, caught only after the fact and remediated as a separate `docs:` catch-up commit (`c47f5c3`) — don't let the gap reopen.
- **"Its unit tests" means logic worth locking in, not one spec file per source file.** The line isn't "does this have branching" — it's **data/business logic vs. view/interaction behavior**. Logic (a computed/derived value, a validation or state-transition rule, a payload-shape decision, a data mapper, a guard/interceptor) is cheap to verify in isolation and a human skimming a rendered page won't reliably notice a regression there — e.g. a silent data-loss bug (a PATCH body quietly omitting a field it shouldn't) looks identical in the UI whether it's correct or broken. View/interaction behavior ("does clicking X render Y," "does a smart page wire its inputs to a child correctly," "does a dialog open/close on the right buttons") — verify manually or via the mocked E2E suite (`e2e/specs/`), don't write a component-harness spec for it; a rendered page proves it just as reliably, without the maintenance cost. **Don't re-prove the same branch across layers either** — before adding (or keeping) a page-orchestration test, check whether an E2E test already exercises that exact scenario with equal precision (exact resulting URL, exact error text — not just "something rendered"); this applies to specs written _before_ the relevant E2E coverage existed too, not just new work. **Exception:** shared, multi-consumer components (`PaginatorBar`, `RatingStars`, `RestaurantGrid`, etc.) keep their own logic tests regardless of any one consumer's E2E coverage — a regression there breaks every consumer, and no single consumer's E2E flow is a reliable safety net for the others. Applied retroactively twice now: commit 10 (2026-08-17: deleted `app.spec.ts`/`app-shell.spec.ts`/`site-footer.spec.ts`, trimmed `main-toolbar.spec.ts`) and again after the mocked E2E suite landed (2026-08-24: deleted `main-toolbar.spec.ts` outright, trimmed `login-page`/`register-page`/`restaurant-detail-page`/`reviews-section`/`restaurant-grid`/`user-menu`/`favorite-button`/`theme-toggle` specs down to logic-only — see `docs/ARCHITECTURE.md`'s "E2E testing" section). Don't reintroduce either pattern a third time.
