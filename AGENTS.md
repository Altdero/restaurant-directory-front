...
- **"Its unit tests" means logic worth locking in, not one spec file per source file.** Write a `.spec.ts` only when there's a real branch, computed, state transition, or integration contract to verify (stores, guards, interceptors, mappers, utils — always). Skip it for a component whose only content is a static template with no inputs/branching (a shell composing children, a footer with a fixed tagline) — asserting hardcoded structure or copy catches no real bugs and just makes a future copy edit fail a test for no reason. When a component mixes both, test the branches and skip the static parts. This was applied retroactively once already (commit 10, 2026-08-17: deleted `app.spec.ts`, `app-shell.spec.ts`, `site-footer.spec.ts`, trimmed one test from `main-toolbar.spec.ts`) — don't reintroduce the pattern.
